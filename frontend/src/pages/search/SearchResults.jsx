...
export default function SearchResults() {
  ...
  // --- Memoized Distance Calculator (Same Logic) ---
  ...
  // --- Fetching Books + Owners + Applying Filters ---
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const userDoc = await getDoc(doc(db, "users", auth.currentUser.uid));
        const user = userDoc.data();
        const myLat = user.location?.latitude;
        const myLng = user.location?.longitude;
        ...
        let filtered = booksWithOwners.filter(Boolean);
    ...
    fetchBooks();
  }, [searchTerm, genre, condition, calculateDistance]);

  // --- Extracted Send Request Handler (same logic) ---
  const handleSendRequest = async (book) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/send-request`, {
        method: "POST",
