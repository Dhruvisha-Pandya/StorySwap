...
def send_email_async(to_email, subject, html_body):
    ...
    def email_thread():
        try:
            print(f"📧 [ASYNC] Starting email thread to: {to_email}")
            print(f"📧 [ASYNC] Time: {datetime.now().isoformat()}")
            
            if not SMTP_EMAIL or not SMTP_PASSWORD:
                print("❌ [ASYNC] SMTP credentials missing")
                return
            ...
            print("🔌 [ASYNC] Connecting to SMTP...")
            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
                print("✅ [ASYNC] Connected to SMTP")
                server.login(SMTP_EMAIL, SMTP_PASSWORD)
            ...
            print(f"❌ [ASYNC] AUTH FAILED: {e}")
        except Exception as e:
            print(f"❌ [ASYNC] ERROR: {str(e)}")
    
    # Start the email in a separate thread
    thread = threading.Thread(target=email_thread)
    thread.daemon = True
    thread.start()
    print("🔄 [ASYNC] Email thread started - main request can now complete")
...
def test_email():
    """TEST THIS ENDPOINT FIRST to check if emails work"""
    try:
        test_email = "dppandya0918@gmail.com"  # ⚠️ CHANGE THIS TO YOUR EMAIL
        print(f"🧪 Starting ASYNC email test to: {test_email}")
        ...
        print("🎉 TEST: Async email started - check your inbox in 1-2 minutes")
        return jsonify({"success": True, "message": "Test email queued! Check your inbox and Render logs."})
...
def send_request():
    try:
        ...
        print("📧 Starting ASYNC email send...")
        send_email_async(
            to_email=lender_email,
            subject=f"Borrow Request for {book_title}",
            html_body=email_body
        ...
        # Return immediately - don't wait for email to complete
        print("✅ Request processed - returning response while email sends in background")
        return jsonify({"success": True, "message": "Request sent! Notification is being delivered."}), 200
...
def respond_request():
    try:
        ...
        print(f"📧 Starting ASYNC {status_text} notification...")
        send_email_async(
            to_email=borrower_email,
            subject=f"Your Borrow Request Was {status_text}",
            html_body=email_body
        ...
        return """
        <script>
            alert("Response recorded. Borrower is being notified.");
            window.close();
        </script>
        """
